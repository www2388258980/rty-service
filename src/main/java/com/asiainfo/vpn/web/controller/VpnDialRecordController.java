...
public class VpnDialRecordController {
    ...
    @ApiImplicitParams(
            value =
                    {
                            @ApiImplicitParam(name = "startDate", value = "拨入日期大于等于该值,前台传字符串,需要满足：yyyy-MM-dd格式",
                                    paramType = "query"),
                            @ApiImplicitParam(name = "endDate", value = "拨入日期小于等于该值,前台传字符串,需要满足：yyyy-MM-dd格式",
                                    paramType = "query"),
    ...
    public OperationResult<List<VpnDialRecord>> getRecords(VpnDialRecord record, String startDate, String endDate,
        ...
        try {
            ...
            e.printStackTrace();
            or = new OperationResult<List<VpnDialRecord>>();
            or.setStatus(OperationResult.STATUS_FAILURE);
            or.setData(null);
